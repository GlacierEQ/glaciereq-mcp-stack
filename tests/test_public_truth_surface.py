...
def test_machine_capability_matches_repository_native_proof() -> None:
    ...
    expected = "policy-gated-local-tool-dispatch"
    assert capabilities["capabilities"] == [expected]
    assert contract["verified_capability"] == expected
    assert state["verified_capability"] == expected
    assert manifest["verified_capability"] == expected
    assert manifest["evidence_state"] == "LOCAL_ALLOWLIST_ROUTER_NOT_EXTERNAL_MCP_DEPLOYMENT"
    assert "Executable MCP server" not in manifest["primary_role"]
